...
describe("lib/UA", function() {
  ...
  describe('UA("uastring")', () => {
    ...
    describe("removes iOS webview browsers from uastring", () => {
      ...
      it("chrome for iOS", () => {
        ...
        const iOS =
          "Mozilla/5.0 (iPhone; U; CPU iPhone OS 5_1_1 like Mac OS X; en-gb) AppleWebKit/534.46.0 (KHTML, like Gecko)  Mobile/9B206 Safari/7534.48.3";
        new UA(chromeIOS);
      ...
      it("opera for iOS", () => {
        const operaIOS =
          "Mozilla/5.0 (iPhone; CPU iPhone OS 7_1_2 like Mac OS X) AppleWebKit/537.51.2 (KHTML, like Gecko) OPiOS/8.0.2.80660 Mobile/11D257 Safari/9537.53";
        const iOS =
          "Mozilla/5.0 (iPhone; CPU iPhone OS 7_1_2 like Mac OS X) AppleWebKit/537.51.2 (KHTML, like Gecko)  Mobile/11D257 Safari/9537.53";
        new UA(operaIOS);
    ...
    describe("removes Electron browsers from uastring to enable them to report as Chrome", () => {
      ...
      it("Electron for OS X", () => {
        ...
        const chrome =
          "Mozilla/5.0 (Macintosh; Intel Mac OS X 10_12_3) AppleWebKit/537.36 (KHTML, like Gecko) WELLMessenger/1.1.0 Chrome/53.0.2785.143 Safari/537.36";
        new UA(electron);
      ...
      it("Electron for Windows", () => {
        ...
        const chrome =
          "Mozilla/5.0 (Windows NT 6.1; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) WELLMessenger/1.1.0 Chrome/53.0.2785.143 Safari/537.36";
        new UA(electron);
        assert.calledOn(spy, electron);
        assert.equal(spy.returned(chrome), true);
      });
    ...
    describe("removes Facebook in-app browsers from uastring", () => {
      ...
      it("Facebook for Android, using Chrome browser", () => {
        ...
        const expected =
          "Mozilla/5.0 (Linux; Android 4.4.2; SCH-I535 Build/KOT49H) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/30.0.0.0 Mobile Safari/537.36";
        new UA(facebook);
        assert.calledOn(spy, facebook);
        assert.equal(spy.returned(expected), true);
      });
  ...
  describe(".getFamily", () => {
    it("returns browser family from useragent", () => {
      ...
      const ios2 = new UA(
        "Mozilla/5.0 (iPad; CPU OS 6_1_3 like Mac OS X) AppleWebKit/536.26 (KHTML, like Gecko) Mobile/10B329 [FBAN/FBIOS;FBAV/6.0.2;FBBV/183159;FBDV/iPad3,1;FBMD/iPad;FBSN/iPhone OS;FBSV/6.1.3;FBSS/2; FBCR/;FBID/tablet;FBLC/en_US;FBOP/1]"
      );
      assert.equal(ios2.getFamily(), "ios_saf");

      const ios3 = new UA(
        "Mozilla/5.0 (iPhone; CPU iPhone OS 6_1_3 like Mac OS X) AppleWebKit/536.26 (KHTML, like Gecko) Mobile/10B329 [FBAN/FBIOS;FBAV/6.0.2;FBBV/183159;FBDV/iPhone4,1;FBMD/iPhone;FBSN/iPhone OS;FBSV/6.1.3;FBSS/2; FBCR/AT&T;FBID/phone;FBLC/en_US;FBOP/1]"
      );
