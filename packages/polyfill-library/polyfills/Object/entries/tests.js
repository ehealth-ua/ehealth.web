...
if (supportsDescriptors) {
  it("does not see a new element added by a getter that is hit during iteration", function() {
    ...
    proclaim.deepEqual(result, [["a", "A"], ["b", "B"]]);
  });
...
it("does not see inherited properties", function() {
  ...
  proclaim.deepEqual(result, [["b", f.b], ["c", f.c]]);
});
...
it("accepts number primitives", function() {
  if (objectKeysWorksWithPrimitives) {
    ...
    proclaim.equal(Object.entries(-0).length, 0, "-0 has zero entries");
    proclaim.equal(
      Object.entries(Infinity).length,
      0,
      "Infinity has zero entries"
    );
    proclaim.equal(
      Object.entries(-Infinity).length,
      0,
    ...
    proclaim.equal(
      Object.entries(Math.PI).length,
      0,
      "Math.PI has zero entries"
    );
  } else {
    this.skip();
  }
...
it("accepts string primitives", function() {
  if (objectKeysWorksWithPrimitives) {
    ...
    proclaim.deepEqual(result, [["0", "a"], ["1", "b"], ["2", "c"]]);
  } else {
    this.skip();
  }
});
...
it("does not include Symbol keys", function() {
  if (hasSymbols) {
    ...
    proclaim.deepEqual(result, [["key", symValue]]);
  } else {
    this.skip();
  }
});
