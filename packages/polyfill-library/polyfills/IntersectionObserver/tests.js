...
before(function(done) {
  ...
  scriptEl.integrity =
    "sha384-LlnC/lp2F2Eof99zkUXeTShqREmtqBk+NThEADCzqlEDjjuhUIS7XXxYd9niLHXh";
...
describe("IntersectionObserver", function() {
  ...
  describe("constructor", function() {
    ...
    if ("thresholds" in IntersectionObserver.prototype) {
      it("instantiates thresholds correctly", function() {
        ...
        io = new IntersectionObserver(noop, { threshold: [1, 0.5, 0] });
        proclaim.deepEqual(io.thresholds, [0, 0.5, 1]);
      });
    }
  ...
  describe("observe", function() {
    ...
    it("handles container elements with non-visible overflow", function(done) {
      var spy = sinon.spy();
    ...
    it("observes multiple targets at multiple thresholds correctly", function(done) {
      var spy = sinon.spy();
    ...
    it("handles zero-size targets within the root coordinate space", function(done) {
      io = new IntersectionObserver(
        function(records) {
          proclaim.equal(records.length, 1);
      ...
      io = new IntersectionObserver(function(records) {
        try {
          ...
          proclaim.equal(
            rootBounds.top,
            0,
            "rootBounds top expected to be " + 0 + " but was " + rootBounds.top
          );
          proclaim.equal(
            rootBounds.left,
            0,
            "rootBounds top expected to be " + 0 + " but was " + rootBounds.left
          );
          proclaim.equal(
            rootBounds.right,
            viewportWidth,
            "rootBounds top expected to be " +
              viewportWidth +
              " but was " +
              rootBounds.right
          );
