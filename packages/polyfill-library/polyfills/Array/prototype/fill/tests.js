...
function compareArray(a, b) {
  if (b.length !== a.length) {
    return proclaim.fail(
      a,
      b,
      "proclaim.deepEqualed an array of length " +
        b.length +
  ...
  for (var i = 0; i < a.length; i++) {
    if (b[i] !== a[i]) {
      return proclaim.fail(
        a[i],
        b[i],
        "proclaim.deepEqualed item in array index " +
          i +
          " to be " +
          b[i] +
          ", received " +
...
it("fills whole array with undefined if passed no arguments", function() {
  proclaim.equal(
    compareArray([1, 2, 3].fill(), [undefined, undefined, undefined]),
    true
  );
});
...
it("works on array-like objects", function() {
  proclaim.deepEqual([].fill.call({ length: 3 }, 4), {
    0: 4,
    1: 4,
    2: 4,
    length: 3
