...
it("finds only the last occurence", function() {
  proclaim.equal(
    Array.prototype.lastIndexOf.call(["foo", "bar", "bar", "bar"], "bar"),
    3
  );
});
...
it("works on array-like objects", function() {
  proclaim.equal(
    Array.prototype.lastIndexOf.call({ 0: 5, 1: 6, length: 2 }, 6),
    1
  );
  proclaim.equal(
    Array.prototype.lastIndexOf.call({ 0: 5, 1: 6, 2: 7, length: 2 }, 7),
    -1
  );
});
