Object.defineProperty(Element.prototype, "placeholder", {
  ...
  set: function(value) {
    if (
      ...
      (/^(input)$/i.test(this.nodeName) &&
        !/^(email|number|password|search|tel|text|url|)$/i.test(
    ...
    var element = this,
      ...
      xPlaceholder = xInput.appendChild(
        document.createElement("ms-placeholder")
      ),
    ...
    element.attachEvent("onkeyup", function() {
      xPlaceholderRuntimeStyle.display = element.value
        ? "none"
        : "inline-block";
    });
