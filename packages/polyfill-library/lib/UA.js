...
function UA(uaString) {
  ...
  let normalized =
    uaString.length < 22 &&
    uaString.match(/^(\w+)\/(\d+)(?:\.(\d+)(?:\.(\d+))?)?$/i);
  if (!normalized) {
    normalized = cache.get(uaString);
  }
  if (normalized) {
    this.ua = new useragent.Agent(
      normalized[1],
      normalized[2],
      normalized[3] || 0,
      normalized[4] || 0
    ...
    if (aliases[this.ua.family]) {
      ...
      if (typeof aliases[this.ua.family] === "function") {
        ...
        Array.isArray(aliases[this.ua.family]) &&
        aliases[this.ua.family].length >= 2
      ) {
        ...
        for (let semverExpr in aliases[this.ua.family]) {
          ...
          if (
            this.ua.satisfies(semverExpr) &&
            Array.isArray(aliases[this.ua.family][semverExpr])
          ) {
...
UA.prototype.isUnknown = function() {
  return (
    Object.keys(baseLineVersions).indexOf(this.ua.family) === -1 ||
    !this.meetsBaseline()
  );
};
