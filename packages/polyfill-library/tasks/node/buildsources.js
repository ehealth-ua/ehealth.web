...
function checkDependenciesExist(polyfills) {
  for (const polyfill of polyfills) {
    for (const dependency of polyfill.dependencies) {
      if (
        !polyfills.some(function(polyfill) {
        ...
        return Promise.reject(
          `Polyfill ${
            polyfill.name
          } depends on ${dependency}, which does not exist within the polyfill-service. Recommended to either add the missing polyfill or remove the dependency.`
        );
      }
    }
  }
...
class Polyfill {
  ...
  loadConfig() {
    return readFile(this.configPath)
      ...
      .then(data => {
        ...
        this.config.hasTests = fs.existsSync(
          path.join(this.path.absolute, "tests.js")
        );
        this.config.isTestable = !(
          "test" in this.config &&
          "ci" in this.config.test &&
          this.config.test.ci === false
        );
        this.config.isPublic = this.name.indexOf("_") !== 0;

        if (fs.existsSync(this.detectPath)) {
          this.config.detectSource =
            fs.readFileSync(this.detectPath, "utf8").replace(/\s*$/, "") || "";
          validateSource(
            `if (${this.config.detectSource}) true;`,
            `${this.name} feature detect from ${this.detectPath}`
          );
        }
      });
  }
    ...
    if (this.config.esversion && this.config.esversion > 5) {
      if (this.config.esversion === 6) {
        ...
        throw {
          ...
          message: `Feature ${this.name} uses ES${
            this.config.esversion
          } but no transpiler is available for that version`
        };
      }
    }
  ...
  writeOutput(root) {
    ...
    return makeDirectory(dest).then(() =>
      Promise.all(
        files
...
Promise.resolve()
  .then(() =>
    Promise.all(
      flattenPolyfillDirectories(src)
  ...
  .then(polyfills =>
    checkForCircularDependencies(polyfills)
      ...
      .then(() =>
        Promise.all(polyfills.map(polyfill => polyfill.writeOutput(dest)))
      )
  )
