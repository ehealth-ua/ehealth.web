...
class Select extends Component {
  ...
  state = {
    ...
    active: (() =>
      this.props.options.filter(item => item.name === this.props.active)[0])()
  };
  ...
  render() {
    const { options = [], placeholder, disabled, labelText } = this.props;
    ...
    return (
      <OuterClick onClick={() => this.setState({ open: false })}>
        <section ref={ref => (this.selectNode = ref)} className={classNames}>
          ...
          >
            <span hidden={activeItem.title} className={styles.placeholder}>
              {placeholder}
            </span>
